class API
  class Accounts < Base
    ...
    post "/facebook_neighbors" do
      control_access :authenticated
      access_token = (current_user.facebook_user? ?
        current_user.oauth2_token : params[:access_token])

      serialize FacebookNeighbors.matches(current_user.community, access_token)
    end

    # TODO: Document these

    post "/gmail_neighbors" do
      control_access :authenticated
      serialize GmailNeighbors.matches(current_user.community, params)
    end
